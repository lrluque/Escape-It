...
public class FirstPersonAudio : MonoBehaviour
{
    ...
    AudioSource[] MovingAudios => new AudioSource[] { walkingAudio, runningAudio, crouchedAudio };
    ...
    void Reset()
    {
        ...
        walkingAudio = GetOrCreateAudioSource("Walking Audio");
    ...
    void FixedUpdate()
    {
        ...
        if (velocity >= velocityThreshold && groundCheck && groundCheck.isGrounded)
        {
            ...
            else if (character.IsRunning)
            {
                ...
                HearingManager.Instance.OnSoundEmitted(gameObject, transform.position, EHeardSoundCategory.EFootstep, 0.5f);
            }
            else
            {
                SetPlayingMovingAudio(walkingAudio);
                HearingManager.Instance.OnSoundEmitted(gameObject, transform.position, EHeardSoundCategory.EFootstep, 0.25f);
            }
        }
        else
        {
