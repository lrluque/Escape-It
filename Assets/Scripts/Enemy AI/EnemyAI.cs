...
public class EnemyAI : MonoBehaviour
{
    ...
    void Update()
    {
        FeedbackDisplay.text = this.GetComponent<GOAPPlanner>().ActiveGoal.GetType().ToString() + 
                                "\n" + this.GetComponent<GOAPPlanner>().ActiveAction.GetType().ToString()
                                + "\nSensors detection: " + Awareness.ActiveTargets.Count.ToString()
                                + "\n" + this.GetComponent<GOAPPlanner>().DebugText;
    }
